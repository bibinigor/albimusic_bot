            ...
            for i in range(90):  # 90 попыток по 10 секунд = 15 минут
                ...
                if status_response.status_code == 200:
                    ...
                    if status == "SUCCESS":
                        ...
                        if audio_data:
                            # Получаем ВСЕ аудио ссылки из sunoData
                            audio_urls = []
                            for i, audio_item in enumerate(audio_data):
                                audio_url = audio_item.get("audioUrl")
                                if audio_url:
                                    audio_urls.append(audio_url)
                                    logger.info(f"[{request_id}]    • Audio URL #{i+1}: {audio_url}")
                            ...
                            if audio_urls:
                                ...
                                logger.info(f"[{request_id}]    • Total audio URLs: {len(audio_urls)}")
                                logger.info(f"[{request_id}]    • Audio URLs JSON: {audio_urls_json}")
                                logger.info(f"[{request_id}]    • Total time: {time.time() - start_time:.2f}s")
                                return audio_urls_json
                    elif status == "ERROR":
