...
def generate_karaoke_from_upload_task(self, user_id, audio_url, task_id=None):
    ...
    try:
        ...
        # Запрос к Suno API для vocal removal
        suno_api_url = config.SUNO_API_URL
        headers = {
            'Authorization': f'Bearer {config.SUNO_API_KEY}',
            'Content-Type': 'application/json'
        ...
        # Используем endpoint для отделения вокала
        payload = {
            'audio_url': audio_url
        }
        ...
        endpoints_to_try = [
            '/api/v1/separate-vocal',
            '/api/v1/vocal-removal',
            '/api/v1/split-stem'
        ]
        ...
        for endpoint in endpoints_to_try:
            try:
                logger.info(f"🔄 Пробую endpoint: {endpoint}")
                response = requests.post(f"{suno_api_url}{endpoint}", headers=headers, json=payload, timeout=300)
                if response.status_code == 200:
        ...
        if not response or response.status_code != 200:
            raise Exception(f"Все endpoints не сработали. Последний статус: {response.status_code if response else 'None'}")

        data = response.json()
        logger.info(f"📊 Ответ API: {data}")

        # Извлекаем instrumental URL (караоке версия)
        # Формат ответа может быть: {vocals_url, instrumental_url} или {stems: [...]}
        instrumental_url = None

        if 'instrumental_url' in data:
            instrumental_url = data['instrumental_url']
        elif 'instrumental' in data:
            instrumental_url = data['instrumental']
        elif 'stems' in data and isinstance(data['stems'], list):
            for stem in data['stems']:
                if 'instrumental' in stem.get('type', '').lower():
                    instrumental_url = stem.get('url')
                    break

        if instrumental_url:
            output_audio_url = json.dumps([instrumental_url])  # Сохраняем как массив
            result_status = 'completed'
            result_message = 'Караоке успешно создано'
        else:
            raise Exception("Instrumental URL не найден в ответе API")
...
def generate_cover_from_upload_task(self, user_id, audio_url, style, task_id=None):
    """Генерация кавера из загруженного файла (upload-cover)"""
    if not task_id:
        task_id = self.request.id
    ...
    try:
        ...
        response = requests.post(f"{suno_api_url}/api/v1/generate/upload-cover", headers=headers, json=payload, timeout=300)
        ...
        if data and 'data' in data and isinstance(data['data'], list):
            ...
            if audio_urls:
                output_audio_url = json.dumps(audio_urls[:2])  # Сохраняем как массив
                result_status = 'completed'
                result_message = 'Кавер успешно создан'
            else:
                raise Exception("Аудио URL не найдены в ответе")
