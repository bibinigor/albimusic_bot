...
async def send_telegram_notification(user_id, task_id, audio_url, is_song=False):
    ...
    try:
        ...
        if language == 'ru':
            ...
            # Создаем инлайн-кнопки (как было раньше)
    ...
    while True:
        try:
            ...
            results = execute_query_sync(
                "SELECT task_id, user_id, prompt, audio_url, status FROM generations WHERE (status = 'completed' AND audio_url IS NOT NULL) OR (status = 'error' AND audio_url IS NOT NULL) ORDER BY created_at DESC LIMIT 10"
            )
