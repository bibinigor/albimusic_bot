...
# Импорт модуля техобслуживания
from maintenance_mode import is_maintenance_mode, MAINTENANCE_MESSAGE

# Обработчики Telegram бота
@dp.message_handler(lambda message: message.text and message.text.lower() in ['/start', 'старт'], state='*')
async def cmd_start(message: types.Message, state: FSMContext):
    # Проверка режима техобслуживания
    if is_maintenance_mode():
        await message.answer(MAINTENANCE_MESSAGE, parse_mode="Markdown")
        return
