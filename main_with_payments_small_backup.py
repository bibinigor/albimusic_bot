...
def generate_suno_music_sync(prompt, is_song=False, custom_mode=False, user_id=None, style=None):
    ...
    # Логируем данные ДО отправки в Suno
...
@celery_app.task(bind=True, name='celery_tasks.generate_music_task', max_retries=3, default_retry_delay=30, time_limit=600, soft_time_limit=480)
...
@celery_app.task(bind=True, name='celery_tasks.generate_song_task', max_retries=3, default_retry_delay=30, time_limit=600, soft_time_limit=480)
...
# Атомарная функция для создания генерации с защитой от race conditions
