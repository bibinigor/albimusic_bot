...
from celery_tasks import celery_app, generate_music_task, generate_song_task, generate_suno_lyrics_sync
...
def get_main_menu_keyboard(user_id=None):
    ...
    markup.add(KeyboardButton("🎧 Примеры песен"))
...
async def process_play_track(callback_query: types.CallbackQuery):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("🎬 Создать видео", callback_data=f"video_{task_id}"),
    ...
    markup.add(InlineKeyboardButton("📢 В канал", callback_data=f"post_{task_id}"))
...
async def process_cover(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id, "🎸 Функция скоро будет доступна!", show_alert=True)
