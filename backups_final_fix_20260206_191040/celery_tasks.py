...
def generate_video_task(self, user_id, original_task_id, task_id=None):
    ...
    try:
        ...
        # Получаем audio_url из оригинальной генерации
        ...
        audio_urls_json = generation[0][0]

        # Парсим JSON массив (может быть строка или уже список)
        try:
            if isinstance(audio_urls_json, str):
                audio_urls = json.loads(audio_urls_json)
            else:
                audio_urls = audio_urls_json
        except:
            raise Exception("Не удалось распарсить audio URL")

        if not audio_urls or len(audio_urls) == 0:
            raise Exception("Аудио URL не найдены")

        # Берем первый трек
        first_audio_url = audio_urls[0]

        # Извлекаем audioId из URL (формат: https://.../{audioId}.mp3)
        audio_id = first_audio_url.split('/')[-1].replace('.mp3', '').replace('.wav', '')
