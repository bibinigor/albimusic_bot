...
def get_admin_stats():
    ...
    try:
        ...
        new_today_result = execute_query_sync("SELECT COUNT(*) as total FROM users WHERE created_at >= CURRENT_DATE")
        ...
        # 6. Успешных генераций (completed)
        ...
        # 8. Оплаченных генераций - ПРАВИЛЬНЫЙ ПОДСЧЕТ через таблицу payments
        ...
        # 9. Приглашенных сегодня (по таблице users)
...
def get_main_menu_keyboard(user_id=None):
    ...
    markup.add(KeyboardButton("🎸 Кавер"))
    markup.add(KeyboardButton("📂 Мои треки"), KeyboardButton("💰 Баланс"))
    markup.add(KeyboardButton("🎧 Примеры песен и промптов"))
...
def get_balance_keyboard(user_id):
    ...
    markup.add(
        ...
        InlineKeyboardButton("🌟 Пригласить друга (2 генерации = 4 трека)", callback_data="invite_friend"),
        InlineKeyboardButton("💳 10 генераций (20 треков) - 490₽", callback_data="pay_490"),
        InlineKeyboardButton("🔥 25 генераций (50 треков) - 990₽", callback_data="pay_990"),
        InlineKeyboardButton("⭐ 60 генераций (120 треков) - 1990₽", callback_data="pay_1990"),
        InlineKeyboardButton("💎 140 генераций (280 треков) - 3990₽", callback_data="pay_3990")
    )
    return markup

def get_admin_keyboard():
    """Упрощенная клавиатура админ-панели (только 2 кнопки)"""
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
...
async def yookassa_webhook(request: Request):
    try:
        ...
        if notification.get('event') == 'payment.succeeded':
            ...
            # Проверяем, не является ли это платежом за разблокировку трека
            if await demo_system.check_unlock_payment(payment_id, payment, bot):
            ...
            if user_id and amount:
                ...
                generations = amount_to_generations.get(amount, 1)  # По умолчанию 1 если сумма неизвестна
                ...
                tracks_count = generations * 2
                try:
                    await bot.send_message(
                        user_id,
                        f"✅ Оплата получена!\n\n"
                        f"💰 На ваш баланс начислено: {generations} генераций ({tracks_count} треков)\n\n"
                        f"🎵 Теперь вы можете создавать музыку!"
                    )
                except Exception as e:
...
@dp.message_handler(lambda message: message.text == "🎤 Караоке", state='*')
async def handle_karaoke_menu(message: types.Message, state: FSMContext):
    """Обработчик кнопки Караоке в главном меню - выбор источника"""
    await state.finish()
    ...
    text = (
        "🎤 <b>КАРАОКЕ</b>\n\n"
        "Караоке — это инструментальная версия БЕЗ вокала.\n"
        "Идеально для исполнения!\n\n"
        "💰 Стоимость: 100₽ (1 генерация)\n\n"
        "<b>Выбери источник:</b>"
    )
...
async def handle_karaoke_from_tracks(callback_query: types.CallbackQuery):
    ...
    if not tracks:
        await bot.send_message(
            user_id,
            "📂 У вас пока нет треков для караоке.\n\n"
...
async def handle_song_examples(message: types.Message, state: FSMContext):
    ...
    text = (
        "🎧 **Примеры песен**\n\n"
        "Слушайте примеры песен, созданных нашим ботом, в официальном канале AL BI Chart!\n\n"
        "🎵 Здесь вы найдете:\n"
        "• Лучшие треки, созданные пользователями\n"
        "• Различные музыкальные жанры\n"
        "• Вдохновение для ваших собственных композиций\n\n"
...
@dp.callback_query_handler(lambda c: c.data == "text_ai", state=CreateSongStates.choosing_text_type)
async def handle_ai_text(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "▪️ для кого / о ком\n"
        "▪️ какие мысли упомянуть\n"
        "▪️ идея которую хочется передать песней\n\n"
        "📩 Всё в одном сообщении — и я создам текст песни!\n\n"
...
@dp.callback_query_handler(lambda c: c.data == "text_own", state=CreateSongStates.choosing_text_type)
...
async def process_song_idea(message: types.Message, state: FSMContext):
    ...
    if not is_admin(user_id):
        ...
        if balance <= 0:
            await message.answer(
                "❌ Недостаточно генераций на балансе.\n\nДобавьте генерации в разделе 💰 Баланс",
    ...
    await message.answer("⏳ Генерирую текст песни... Это займет около 30-60 секунд...")

    # Вызываем Suno API для генерации текста (синхронная функция, выполняется в отдельном потоке)
    import asyncio
    ...
    try:
        ...
        await state.update_data(lyrics=generated_lyrics, is_ai_generated=True)

        # Показываем сгенерированный текст
        MAX_MSG_LENGTH = 4000
        if len(generated_lyrics) <= MAX_MSG_LENGTH:
            await message.answer(f"✨ **ГОТОВО! ВОТ ТВОЙ ТЕКСТ:**\n\n{generated_lyrics}", parse_mode="Markdown")
        else:
            # Разбиваем на части если длинный
            await message.answer(f"✨ **ГОТОВО! ВОТ ТВОЙ ТЕКСТ:**\n\n{generated_lyrics[:MAX_MSG_LENGTH]}", parse_mode="Markdown")
            remaining = generated_lyrics[MAX_MSG_LENGTH:]
            while remaining:
                chunk = remaining[:MAX_MSG_LENGTH]
                await message.answer(chunk, parse_mode="Markdown")
                remaining = remaining[MAX_MSG_LENGTH:]

        # Сразу переходим к выбору жанра
...
async def process_payment(callback_query: types.CallbackQuery):
    ...
    # Определяем сумму и количество генераций
    payment_data = {
        'pay_490': (490, 10, "10 генераций (20 треков)"),
        'pay_990': (990, 25, "25 генераций (50 треков)"),
        'pay_1990': (1990, 60, "60 генераций (120 треков)"),
        'pay_3990': (3990, 140, "140 генераций (280 треков)")
    }

    if callback_query.data in payment_data:
        amount, generations, description = payment_data[callback_query.data]
    else:
    ...
    if payment and payment.get('confirmation', {}).get('confirmation_url'):
        ...
        await bot.send_message(
            user_id,
            f"💳 *Оплата {amount}₽*\n\n"
            f"🎵 Вы получите: {generations} генераций ({tracks_count} треков)\n\n"
...
async def process_invite_friend(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(user_id, "Чтобы получить 2 генерации (4 трека) бесплатно, скопируйте сообщение ниже и отправьте его другу 👇")
    
    ready_text = f"""Привет! Я нашел крутого бота для создания музыки с помощью Искусственного Интеллекта. Попробуй! Там первая генерация песни в подарок. Бот может просто музыку написать по описанию, а может реальную песню, если ему стихи загрузишь.
Прикольная штука :)
...
async def process_admin_stats(callback_query: types.CallbackQuery):
    ...
    # Формируем текст статистики с проверкой наличия полей
...
🎵 Всего генераций: {stats.get("total_generations", 0)} шт
✅ Успешных: {stats.get("success_rate", 0)}%

💰 Оплаченных за 7 дней: {stats.get("paid_7days", 0)}
⏰ Оплаченных за 24 часа: {stats.get("paid_24h", 0)}
📊 Всего оплаченных: {stats.get("paid_total", 0)}

👥 Приглашенных сегодня: {stats.get("invited_today", 0)}"""

    # Добавляем дополнительную информацию о платежах
    if stats.get("paid_total", 0) == 0:
        text += "\n\n⚠️ *Внимание:* Нет успешных платежей (status = succeeded)"
...
async def process_music_style(message: types.Message, state: FSMContext):
    if len(message.text) > MAX_STYLE_LENGTH:
        await message.answer(f"❌ Слишком длинное описание. Максимум {MAX_STYLE_LENGTH} символов.")
        return
    style = message.text
    ...
    if balance_num <= 0 and not is_admin(user_id):
        await message.answer("❌ Зайдите пожалуйста в баланс и оплатите генерацию либо пригласите друга и мы подарим вам 2 генерации бесплатно!", reply_markup=get_main_menu_keyboard(user_id))
        await state.finish()
        return
    ...
    # Получаем информацию о бесплатных генерациях из БД
    result = execute_query_sync(
        "SELECT balance, free_generation_used FROM users WHERE user_id = %s",
        (user_id,)
    )
    user_data = result[0] if result else None
    
    # Определяем тип генерации на основе данных из БД
    if user_data:
        balance = user_data[0]  # balance (integer)
        free_generation_used = user_data[1]  # free_generation_used (boolean)
        
        # Проверяем доступность бесплатной генерации
        if not free_generation_used and not is_admin(user_id):
            mark_free_generation_used(user_id)
            is_free = True
        elif balance > 0:
            is_free = False
        else:
            await message.answer("❌ Недостаточно генераций. Используйте /premium для покупки.")
            return
    else:
...
async def process_play_track(callback_query: types.CallbackQuery):
    ...
    # Получаем audio_url из БД
    result = execute_query_sync(
        "SELECT audio_url, prompt FROM generations WHERE task_id = %s AND user_id = %s",
        (task_id, user_id)
    )
    ...
    audio_url_raw, prompt = result[0]

    # Парсим JSON если 2 ссылки
    import json
    urls = []
    ...
    for idx, url in enumerate(urls[:2], 1):
        try:
            await bot.send_audio(user_id, url, caption=f"🎼 Версия {idx}", title=f"AI Music v{idx}", performer="ALBI Music")
        except Exception as e:
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("🎤 Караоке", callback_data=f"karaoke_{task_id}"),
        InlineKeyboardButton("🎸 Создать кавер", callback_data=f"cover_{task_id}")
    )
    markup.add(
        InlineKeyboardButton("🔗 Отправить другу", callback_data=f"share_{task_id}"),
        InlineKeyboardButton("📢 В канал", callback_data=f"post_{task_id}")
    )

    await bot.send_message(user_id, "💎 **Дополнительные возможности:**", reply_markup=markup, parse_mode="Markdown")
...
async def process_repeat_track(callback_query: types.CallbackQuery):
    ...
    if not is_admin(user_id):
        balance = get_balance_number(user_id)
        if balance <= 0:
            await bot.send_message(user_id, "❌ Недостаточно генераций на балансе")
            return
        # Списываем генерацию
        execute_query_sync("UPDATE users SET balance = balance - 1 WHERE user_id = %s", (user_id,))
...
async def process_custom_genre(message: types.Message, state: FSMContext):
    """Получили описание своего варианта жанра"""
...
async def process_video(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id, "🎬 Функция скоро будет доступна!", show_alert=True)

# Обработчик загрузки аудио для караоке
...
async def process_karaoke_audio_upload(message: types.Message, state: FSMContext):
    """Обработка загруженного аудио для караоке"""
    user_id = message.from_user.id
    ...
    if user_id != 338544009:
        ...
        if balance < 1:
            await message.answer(
                ...
                "💰 Стоимость караоке: 100₽ (1 генерация)",
                parse_mode="Markdown",
    ...
    try:
        ...
        logging.info(f"🎤 Караоке задача создана: {new_task_id} для пользователя {user_id}, URL: {file_url}")
...
async def process_cover_audio_upload(message: types.Message, state: FSMContext):
    ...
    if user_id != 338544009:
        ...
        if balance < 2:
            await message.answer(
                "❌ *Недостаточно генераций!*\n\n"
                "💰 Стоимость кавера: 200₽ (2 генерации)",
                parse_mode="Markdown",
                reply_markup=get_balance_keyboard(user_id)
            )
            await state.finish()
...
@dp.callback_query_handler(lambda c: c.data.startswith('karaoke_'))
async def process_karaoke(callback_query: types.CallbackQuery):
    """Обработчик создания караоке-версии"""
    await bot.answer_callback_query(callback_query.id)
    user_id = callback_query.from_user.id
    ...
    if user_id != 338544009:
        ...
        if balance < 1:
            await bot.send_message(
                user_id,
                "❌ *Недостаточно генераций для создания караоке!*\n\n"
                "💰 Стоимость: 100₽ (1 генерация)\n\n"
    ...
    await bot.send_message(
        user_id,
        "🎤 *Создаю караоке-версию...*\n\n"
        "Инструментальный трек будет готов через 1-2 минуты!",
        parse_mode="Markdown"
    )

    logging.info(f"🎤 Караоке задача создана: {new_task_id} для пользователя {user_id}")

@dp.callback_query_handler(lambda c: c.data.startswith('video_'))
async def process_video(callback_query: types.CallbackQuery):
    """Обработчик создания видеоклипа"""
    await bot.answer_callback_query(callback_query.id)
    user_id = callback_query.from_user.id
    task_id = callback_query.data.replace('video_', '')
    ...
    if user_id != 338544009:
        ...
        if balance < 2:
            await bot.send_message(
                user_id,
                "❌ *Недостаточно генераций для создания видеоклипа!*\n\n"
                "💰 Стоимость: 200₽ (2 генерации)\n\n"
                "Пополните баланс или пригласите друга 🎁",
                parse_mode="Markdown",
                reply_markup=get_balance_keyboard(user_id)
        ...
        # Списываем 2 генерации
        update_user_balance(user_id, -2)

    # Получаем аудио ID из оригинальной генерации
    ...
    if not generation or not generation[0]:
        await bot.send_message(
            user_id,
            "❌ Не удалось найти оригинальный трек. Попробуйте снова.",
            parse_mode="Markdown"
        )
        return

    # Создаем новую задачу Celery для видео
    new_task_id = str(uuid.uuid4())
    ...
    logging.info(f"🎬 Видео задача создана: {new_task_id} для пользователя {user_id}")

@dp.callback_query_handler(lambda c: c.data.startswith('cover_'))
async def process_cover(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if user_id != 338544009:
        ...
        if balance < 2:
            await bot.send_message(
                user_id,
                "❌ *Недостаточно генераций для создания кавера!*\n\n"
                "💰 Стоимость: 200₽ (2 генерации)\n\n"
                "Пополните баланс или пригласите друга 🎁",
                parse_mode="Markdown",
                reply_markup=get_balance_keyboard(user_id)
    ...
    # Сохраняем task_id в FSM для последующего использования
    await state.update_data(cover_original_task_id=task_id)
...
async def process_cover_genre(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Получаем оригинальный task_id из FSM
    data = await state.get_data()
    ...
    celery_task = generate_cover_task.apply_async(
        args=[user_id, original_task_id, selected_genre],
        kwargs={'task_id': new_task_id},
        queue='generation'
    )
    ...
    await bot.send_message(
        user_id,
        f"🎸 *Создаю кавер в стиле {selected_genre}...*\n\n"
        "Новая версия будет готова через 1-2 минуты!",
        parse_mode="Markdown"
    )

    await state.finish()
    logging.info(f"🎸 Кавер задача создана: {new_task_id} для пользователя {user_id}, жанр: {selected_genre}")
...
async def process_cover_custom_genre(message: types.Message, state: FSMContext):
    ...
    new_task_id = str(uuid.uuid4())
    celery_task = generate_cover_task.apply_async(
        args=[user_id, original_task_id, custom_genre],
        kwargs={'task_id': new_task_id},
        queue='generation'
    )
    ...
    await state.finish()
    logging.info(f"🎸 Кавер задача создана: {new_task_id} для пользователя {user_id}, жанр: {custom_genre}")
...
async def process_post_to_channel(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Получаем audio_url из базы данных
    try:
        result = execute_query_sync(
        ...
        # Если это JSON массив (2 ссылки), берем первую
        import json
        if isinstance(audio_url_raw, str) and audio_url_raw.startswith('['):
            try:
                urls = json.loads(audio_url_raw)
                if isinstance(urls, list) and len(urls) > 0:
                    audio_url = urls[0]  # Берем первую ссылку
                else:
...
async def process_generation_mode(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if generation_type == "song":
        ...
        if not lyrics:
            ...
            await state.finish()
            return
# [УДАЛЕНО] Повторный импорт (используется глобальный)
