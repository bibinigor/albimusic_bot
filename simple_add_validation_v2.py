...
"""
═══════════════════════════════════════════════════════════════════════
ПРОСТОЕ ТОЧЕЧНОЕ ДОБАВЛЕНИЕ С ПРАВИЛЬНЫМИ ОТСТУПАМИ v2
═══════════════════════════════════════════════════════════════════════

ИСПРАВЛЕНИЯ:
1. ✅ Автоматическое определение уровня отступов
2. ✅ Генерация кода с корректными отступами
3. ✅ Учет контекста (if, функции и т.д.)
...
BACKUP_DIR = "/root/albimusic-bot/backups_simple_v2"
...
def get_indent_level(line):
    """
    Определяет количество пробелов в отступе строки
    
    Args:
        line (str): Строка кода
    
    Returns:
        int: Количество пробелов в начале строки
...
def create_indented_lines(base_indent, code_lines):
    """
    Создает список строк с правильными отступами
    
    Args:
        base_indent (int): Базовый уровень отступа
        code_lines (list): Список строк кода (без отступов)
    
    Returns:
        list: Список строк с правильными отступами
    
    Пример:
        base_indent = 8
        code_lines = ["# Comment", "if x:", "    return"]
        →
        ["        # Comment", "        if x:", "            return"]
    """
    result = []
    ...
    for line in code_lines:
        if not line.strip():  # Пустая строка
            ...
            # Определяем дополнительный отступ внутри строки
            line_indent = get_indent_level(line)
            # Применяем базовый отступ + внутренний отступ
...
def add_mp3_validation(lines):
    ...
    while i < len(lines):
        ...
        if not added and '• Total time:' in line and 'time.time() - start_time' in line:
            ...
            log(f"   Уровень отступа: {indent_level} пробелов", "ℹ️")
            
            # Проверяем следующую строку - должна быть return audio_url
            if i + 1 < len(lines) and 'return audio_url' in lines[i + 1]:
                log(f"🔍 Найден return audio_url на строке {i+2}", "🔍")
                
                # Создаем код с правильными отступами
                validation_code = create_indented_lines(indent_level, validation_code_template)
                
                # Вставляем код ПЕРЕД return
                for code_line in validation_code:
                    new_lines.append(code_line)
                
                added = True
                log("✅ Код проверки MP3 добавлен с правильными отступами", "✅")
        
        i += 1
    
    if not added:
        log("⚠️  Не удалось найти место для вставки проверки MP3", "⚠️")
        log("   Ищем: logger.info(f\"[{request_id}]    • Total time:\")", "ℹ️")
        return None
...
def add_style_translation(lines):
    ...
    while i < len(lines):
        ...
        if not added and '• User ID:' in line and '{user_id}' in line:
            ...
            # Ищем следующую строку с headers (в пределах 5 строк)
            next_headers_line = -1
            ...
            if next_headers_line != -1:
                ...
                # Создаем код с правильными отступами
                translation_code = create_indented_lines(indent_level, translation_code_template)
                
                # Копируем строки между User ID и headers
                for j in range(i + 1, next_headers_line):
                    new_lines.append(lines[j])
                
                # Вставляем код перевода ПЕРЕД headers
                for code_line in translation_code:
                    new_lines.append(code_line)
                ...
                added = True
                log("✅ Код перевода стиля добавлен с правильными отступами", "✅")
        
        i += 1
    
    if not added:
        log("⚠️  Не удалось найти место для вставки перевода стиля", "⚠️")
        log("   Ищем: logger.info(f\"[{request_id}]    • User ID:\")", "ℹ️")
        return None
...
def main():
    print("\n" + "═" * 80)
    print("  ПРОСТОЕ ДОБАВЛЕНИЕ ВЫЗОВОВ ФУНКЦИЙ v2 (с правильными отступами)")
    print("═" * 80 + "\n")
