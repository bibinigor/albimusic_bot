...
def generate_suno_music_sync(prompt, is_song=False, custom_mode=False, user_id=None, style=None):
    ...
    if is_song:
        if custom_mode:
            data = {
                "prompt": prompt, "tags": style if style else "pop",
                "customMode": True,
                "instrumental": False,
                "model": "V5",
...
def generate_song_task(self, user_id, lyrics, style, custom_mode=False):
    ...
    try:
        # Объединяем стиль и текст для Suno API
        ...
        logger.info(f"✅ Генерация песни завершена для {user_id}")
        ...
        logger.error(f"❌ Ошибка генерации песни для {user_id}: {e}")
        # Сохраняем ошибку в БД
        save_generation_task_sync(
            user_id=user_id,
            task_id=task_id,
